...
def login_and_configure_time_interval(playwright, deviceID):
    ...
    page.get_by_role("textbox", name="Enter Command").click()
    page.get_by_role("textbox", name="Enter Command").fill("B200")
    page.get_by_role("button", name="save").click()
...
def test_check_b200_pattern(serial_connection, record_property):
    ...
    # Step 3: Launch UI to send B200
    with sync_playwright() as playwright:
        login_and_configure_time_interval(playwright, device_id)

    # Step 4: Enable receive logs
    send_cli_command(ser, 'tr 2', wait=60)
    send_cli_command(ser, 'blemode', wait=7,
                    expected=r"B-Sensor network mode is: (UP|DOWN)",
    regex=True
    )

    # Step 5: Check for B200
    expected_pattern = "B200"
    ...
    assert found, "Expected pattern 'B200' not found in the second row payload."
