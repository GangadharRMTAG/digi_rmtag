...
def send_cli_command(ser, cmd, wait=1, expected=None, partial_match=False):
    log_uart(f"\n>> Sending: {cmd}")
    ser.write((cmd + '\n').encode())
    time.sleep(wait)
    ...
    if expected:
        cleaned_response = strip_ansi_and_timestamp(response)
        if partial_match:
            return expected in cleaned_response
        else:
            return expected == cleaned_response.strip()

    return True  # If no expected value given, just return True
...
def login_and_configure_time_interval(playwright, deviceID):
    ...
    page.get_by_role("textbox", name="Enter Command").click()
    page.get_by_role("textbox", name="Enter Command").fill("B201")
    page.get_by_role("button", name="save").click()
...
def test_check_b201_pattern(serial_connection, record_property):
    ...
    # Step 3: Launch UI to send B201
    with sync_playwright() as playwright:
        login_and_configure_time_interval(playwright, device_id)

    # Step 4: Enable receive logs
    send_cli_command(ser, 'tr 2', wait=60)
    send_cli_command(ser, 'blemode', wait=7, expected=r"B-Sensor network mode is: (UP|DOWN)",
    regex=True
    )
    # Step 5: Check for B201
    expected_pattern = "B201"
    csv_file_path = snowflake_login_and_download_csv(device_id, expected_pattern)

    # Step 6: Check second row payload
    found = False
    ...
    assert found, "Expected pattern 'B201' not found in the second row payload."
    ...
    log_uart("[✓] B201 pattern test completed successfully.")