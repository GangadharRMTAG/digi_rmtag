...
def test_GatewayCanStartDFUWithB2Sensor(serial_connection, record_property):
    ...
    # Initial configuration with expected output validation
    ...
    # 1st Command: C20407060600
    with sync_playwright() as playwright:
        login_and_send_command(playwright, device_id, "C20407060600")

    time.sleep(30)
    send_cli_command(ser, 'tr 2', wait=30)

    # 2nd Command: BB0306060000
    with sync_playwright() as playwright:
        login_and_send_command(playwright, device_id, "BB0306060000")

    time.sleep(30)
    send_cli_command(ser, 'tr 2', wait=30)

    send_cli_command(ser, 'blemode down', wait=2,
