...
def test_OldRecordHandling(serial_connection, record_property):
    ...
    payload = "0311666000000134915483556E40ED4E0F5206"
    for i in range(10):
        assert send_cli_command(ser, f'zpdummy {payload}', wait=2,
    ...
    assert send_cli_command(ser, 'sfstatus', wait=2,)
    assert send_cli_command(ser, 'tr 2', wait=2,)
    assert send_cli_command(ser, 'sfstatus', wait=2,
                            expected="S&F contains 0 items in the store and 0 items in the priority queue")
    assert send_cli_command(ser, 'tr 0', wait=2,
                            expected="Disabling transmissions to Core.")
    payload = "0311666000000134915483556E40ED4E005206"
    for i in range(10):
        assert send_cli_command(ser, f'zpdummy {payload}', wait=2,
                            expected="zpoint: Successfully pushed packet to S&F")
    assert send_cli_command(ser, 'sfstatus', wait=2,)
    ...
    assert send_cli_command(ser, 'tr 0', wait=2,
                            expected="Disabling transmissions to Core.")
    payload = "0311666000000134915483556E40ED4CD45206"
    for i in range(10):
        assert send_cli_command(ser, 'zpdummy 0311666000000134915483556E40ED4CD45206', wait=2,
                            expected="zpoint: Successfully pushed packet to S&F")
