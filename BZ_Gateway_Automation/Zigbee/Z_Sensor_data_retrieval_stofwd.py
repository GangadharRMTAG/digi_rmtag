...
def test_ZSensorDataRetrieval(serial_connection, record_property):
    ...
    assert send_cli_command(ser, 'sfstatus', wait=2, 
                            expected="0 items in the store and 0 items in the priority queue")
    assert send_cli_command(ser, 'zpmode down', wait=2,
                     expected="Zpoint mode set to: DOWN")
    send_cli_command(ser, 'zpmode', wait=2)
    time.sleep(300)
