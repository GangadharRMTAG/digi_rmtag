...
def login_and_configure_time_interval(playwright, deviceID):
    ...
    page.get_by_role("row", name="D5245C8 5260 Gangadhar").get_by_role("checkbox").click()
    ...
    page.get_by_role("textbox", name="Enter Command").click()
    page.get_by_role("textbox", name="Enter Command").fill("C00C")
    page.get_by_role("button", name="save").click()
...
def test_DeviceCFG_OTA(serial_connection, record_property):
    ...
    send_cli_command(ser, 'vbv set port 7', wait=5)
    send_cli_command(ser, 'upload 30 AC Powered Interval', wait=5)
    send_cli_command(ser, 'config print', wait=2)

    with sync_playwright() as playwright:
        login_and_configure_time_interval(playwright, device_id)
        expected_pattern = "C00C"
