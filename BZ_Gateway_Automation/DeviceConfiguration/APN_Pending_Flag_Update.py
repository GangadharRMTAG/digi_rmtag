...
def test_APN_Pending_Flag_Update(serial_connection, record_property):
    ...
    send_cli_command(ser, 'network get_apn', wait=5)
    send_cli_command(ser, 'network set_apn super', wait=5)
    send_cli_command(ser, 'moff', wait=10)
    send_cli_command(ser, 'mon', wait=10)
    send_cli_command(ser, 'network get_apn', wait=5)
    send_cli_command(ser, 'config print', wait=2)
    send_cli_command(ser, 'reset n', wait=2)
    time.sleep(10)  # Wait for device to reset
    apply_standard_device_config(ser, password)
    send_cli_command(ser, 'network get_apn', wait=3)
    send_cli_command(ser, 'config print', wait=2)
