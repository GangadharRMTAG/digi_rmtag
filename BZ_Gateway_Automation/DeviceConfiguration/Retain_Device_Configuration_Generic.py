...
def test_RetainDeviceConfiguration(serial_connection, record_property):
    ...
    send_cli_command(ser, 'upload 20 AC Powered Interval', wait=2)
    send_cli_command(ser, 'config print', wait=2)

    send_cli_command(ser, 'config reset -f', wait=2)
