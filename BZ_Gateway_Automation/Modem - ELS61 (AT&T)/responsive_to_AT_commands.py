...
def test_responsive_to_AT_commands(serial_connection):
    ...
    _, passed = send_cli_command(ser, 'mon', wait=50, expected_log="modem: Modem is online")
    # assert passed, "Failed to turn on modem."

    _, passed = send_cli_command(ser, 'at AT', wait=4, expected_log="modem: ERRLOG:[33] Unhandled modem response: OK")
    assert passed, "AT command response not as expected."

    _, passed = send_cli_command(ser, 'moff', wait=5, expected_log="modem: De-registering from network")
    assert passed, "Failed to turn off modem."
