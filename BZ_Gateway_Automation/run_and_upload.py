...
def run_pytest_and_upload():
    ...
    pytest_cmd = ["pytest", "--junitxml=reports/junit-report.xml", "B2_Sensor/Gateway_can_start_DFU_with_B2_Sensor.py"]
...
def open_testrail_dashboard():
    with sync_playwright() as playwright:
        ...
        # time.sleep(2)
        # page.get_by_test_id("navigateToRunsButton").click()
        time.sleep(2)
        page.get_by_role("link", name="Pytest Testrail").first.click()
        time.sleep(1)
        page.get_by_role("link", name="test_GatewayCanStartDFUWithB2Sensor").click()
        time.sleep(10)
        context.close()
        browser.close()
